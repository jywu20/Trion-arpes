...
let 
    ...
    arrows!(ax, [binding_starting_bar_pos], [exciton.E_g], [0.0], [-exciton.E_B], color=:black, arrowsize=10)
    ...
    lines!(ax, vec(sum(Akω_total, dims=1)), ω_list, color=center_line_color)
    ylims!(ax, (minimum(ω_list_plot), maximum(ω_list_plot)))
...
let 
    ...
    lines!(ax, [binding_starting_bar_pos - binding_starting_bar_width, binding_starting_bar_pos + binding_starting_bar_width], [exciton.E_g, exciton.E_g], color=:black)
    arrows!(ax, [binding_starting_bar_pos], [trion.E_g], [0.0], [-trion.E_B - E_residue_correction], color=:black, arrowsize=10)
    text!(ax, binding_starting_bar_pos + binding_annotation_displacement, trion.E_g - (trion.E_B + E_residue_correction) / 2, text=rich("IE = ", rich("E", subscript("B,T", font=:regular), font=:italic), "+", rich("E", subscript("hh", font=:regular), font=:italic))) 
...
save("ehh-display-very-compact-2-residue-energy-distribution-ionization-integrated.png", f)
f