...
let 
    ...
    ##########################################
    #region Plotting the exciton ARPES intensity summed over the frequency variable
    
    ax = Axis(gl[2, 1], xticks=[-0.4, 0.0, 0.4])
    lines!(ax, kx_list, vec(sum(Akω_total, dims=2)))
    hidedecorations!(ax, ticks = false, ticklabels = false, label = false)
    Label(gl[2, 1, Bottom()], "Momentum (Å⁻¹)", padding=(0, 0, 0, caption_padding))
    ...
    ##########################################
    #region Plotting the exciton ARPES intensity summed over the momentum variable
    ...
    Akω_full_grid = exciton_ARPES_eh(exciton, Q, k1_grid, ω_list, Ak, broaden)
    lines!(ax, vec(sum(Akω_full_grid, dims=1)), ω_list)
    ylims!(ax, (minimum(ω_list_plot), maximum(ω_list_plot)))
    hidexdecorations!(ax, ticks = false, ticklabels = false, label = false)
    hideydecorations!(ax)
    ...
    rowsize!(gl, 1, Fixed(230))
    colsize!(gl, 2, Fixed(50))
...
let 
    ...
    ##########################################
    #region Plotting the trion ARPES intensity with the momentum variable summed over
    ...
    Akω_full_grid = trion_ARPES_ehh(trion, P, k1_grid, k1_grid, ω_list, Ak1k2, broaden)
    ...
    #ax = Axis(gl[1, 3], aspect=0.25, xticks=([0.0], [" "]), xtickcolor=:white)
    #hideydecorations!(ax)
    ...
    #hidedecorations!(ax, ticks = false, ticklabels = false, label = false)
    ##hidexdecorations!(ax, label = false)
    #colsize!(gl, 3, Aspect(1, 0.25))
    colgap!(gl, 0)
    colsize!(gl, 2, Fixed(50))

    #Label(gl[1, 2, Bottom()], "Intensity", tellwidth=false, tellheight=false, padding=(0, 0, 0, caption_padding))
...
let 
    ...
    ax = Axis(f[1, 3], xticks=[-0.4, 0.0, 0.4])
    ...
    colsize!(f.layout, 3, Aspect(1, 0.7))
    Label(f[1, 3, TopLeft()], "(f)", padding = (0, 15, 15, 0))
    Label(f[1, 3, Bottom()], "Momentum (Å⁻¹)", tellwidth=false, tellheight=false, padding=(0, 0, 0, caption_padding))

    #endregion
    ##########################################
end

save("ehh-no-momentum-display-compact-integrated.png", f)
f