...
files = ["eigenval_$(i)_unlikespin_plus_v_new" for i in 0:13]
Q_list = [
    [-0.02, -0.02],
    [-0.015, -0.015],
    [-0.01, -0.01],
...
eig_matrix .+= shift
eig_matrix[2:3, :] .+= shift_2p_unlike
eig_matrix[2:3, :] .-= (eig_matrix[2:3, :] .- eig_matrix[2, 1]) * 0.2
eig_matrix[4:end, :] .+= shift_2s_unlike
...
let first_n = 3
    ...
    for j in 1:min(first_n, size(eig_matrix, 1))
        scatter!(ax, Q_length_list .+ w, eig_matrix[j, eachindex(Q_length_list)])
    end
    ...
    Qxs = LinRange(-0.2 + w,0.2 + w, 100)
    
    ex = Homogeneous2DExciton(Q_length_list, eig_matrix[1, eachindex(Q_length_list)], shift=@SVector [w, 0.0])
    lines!(ax, Qxs, map(Q -> E_exciton(ex, @SVector [Q, 0.0]), Qxs))

    ex = Homogeneous2DExciton(Q_length_list, eig_matrix[2, eachindex(Q_length_list)], shift=@SVector [w, 0.0])
    lines!(ax, Qxs, map(Q -> E_exciton(ex, @SVector [Q, 0.0]), Qxs))


    ex = Homogeneous2DExciton(Q_length_list, eig_matrix[3, eachindex(Q_length_list)], shift=@SVector [w, 0.0])
    lines!(ax, Qxs, map(Q -> E_exciton(ex, @SVector [Q, 0.0]), Qxs))
