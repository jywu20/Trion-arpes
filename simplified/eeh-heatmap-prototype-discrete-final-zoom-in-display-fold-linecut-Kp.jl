...
let f = Figure(size=(600, 400))
    ...
    text!(ax1, maximum(kx_list) - 0.1, 2E_g - trion.E_B - minimum(eig_matrix_0[1, :]) - 0.03, text="1s")
    text!(ax1, maximum(kx_list) - 0.1, 2E_g - trion.E_B - minimum(eig_matrix_0[3, :]) - 0.03, text="2p")
    text!(ax1, maximum(kx_list) - 0.1, 2E_g - trion.E_B - minimum(eig_matrix_0[7, :]) - 0.03, text="2s")
    ...
    save("eeh-heatmap-prototype-discrete-final-zoom-in-display-fold-linecut-Kp.png", f)
