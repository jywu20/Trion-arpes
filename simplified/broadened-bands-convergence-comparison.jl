...
iK, iK′ = open("MoS2-K-Kp-96-bands.dat") do f
    iK = 0
    iK′ = 0
    readline(f)
    readline(f)
...
scatter!(ax, k_list_GW, E_c_GW, color=colorant"seagreen2")
lines!(ax, kx_list, -E_v1_list)
scatter!(ax, k_list_GW, E_v_GW, color=colorant"seagreen2")
...
iK, iK′ = open("MoS2-K-Kp-6000-bands.dat") do f
    iK = 0
    iK′ = 0
    readline(f)
    readline(f)
...
scatter!(ax, k_list_GW, E_v_GW, color=colorant"olivedrab4")
scatter!(ax, k_list_GW, E_c_GW, color=colorant"olivedrab4")

save("effective-mass-benchmark-convergence.png", f)
