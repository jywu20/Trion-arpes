...
for iS in 1:30
    let f = Figure(size=(1500, 500))
        ...
        Avck_overlap_trion = map(1 : size(rk)[2]) do ik
            Ak1k2((@SVector [0.0, 0.0]), SVector{2}(rk[1:2, ik])) * Avck_iS_first_two_bands[ik]
        end
        ...
        ax = Axis(f[1, 2])
        scatter!(ax, rk[1, :], rk[2, :], color=imag.(Avck_overlap_trion), 
            colormap=cgrad(:balance),
            colorrange=(-intensity_limit, intensity_limit),
        )
        ...
        ax = Axis(f[1, 3])
        A_T = map(ik -> Ak1k2((@SVector [0.0, 0.0]), SVector{2}(rk[1:2, ik])), 1 : size(rk)[2])
        intensity_limit = maximum(A_T)
        scatter!(ax, rk[1, :], rk[2, :], color=A_T, 
            colormap=cgrad(:balance),
            colorrange=(-intensity_limit, intensity_limit),
        )
        ...
        save("exciton-0-patch-overlap-$iS.png", f)
