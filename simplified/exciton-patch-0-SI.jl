...
function fetch_Avck(Avck, iS)
    ...
    Avck_iS_first_two_bands
end

set_theme!(fontsize=20)
f = Figure(size=(1600, 400))

A = real.(fetch_Avck(Avck, 2))
ax = Axis(f[1, 1], title="1s", titlefont=:regular,
    ylabel=rich("k", subscript("y", font=:regular), rich(" (Å⁻¹)", font=:regular), font=:italic,),
    yticks=([-0.2, 0.0, 0.2], ["K-0.2", "K", "K+0.2"]),
    xticks=([-0.2, 0.0, 0.2], ["K-0.2", "K", "K+0.2"]),
    xtickalign = 1.0,
    ytickalign = 1.0,
)
intensity_limit = maximum(abs.(A))
scatter!(ax, rk[1, :], rk[2, :], color=A,
...
A = real.(fetch_Avck(Avck, 6))
ax = Axis(f[1, 2], title="2p", titlefont=:regular,
        xtickalign = 1.0,
        ytickalign = 1.0,
        xticks=([-0.2, 0.0, 0.2], ["K-0.2", "K", "K+0.2"]),
    )
...
A = imag.(fetch_Avck(Avck, 6))
ax = Axis(f[1, 3], title="2p", titlefont=:regular,
        xtickalign = 1.0,
        ytickalign = 1.0,
        xticks=([-0.2, 0.0, 0.2], ["K-0.2", "K", "K+0.2"]),
    )
...
A = real.(fetch_Avck(Avck, 10))
ax = Axis(f[1, 4], title="2s", titlefont=:regular,
        xtickalign = 1.0,
        ytickalign = 1.0,
        xticks=([-0.2, 0.0, 0.2], ["K-0.2", "K", "K+0.2"]),
    )
...
Label(f[1, 2:3, Bottom()], rich("k", subscript("x", font=:regular), rich(" (Å⁻¹)", font=:regular), font=:italic,), padding=(0, 0, 0, 40))

save("exciton-0-patch-SI.png", f)
