...
label_shift = 0.05
f = Figure(size=(600, 600), figure_padding=0.0)
...
Label(f[2, 1:3, Bottom()], rich(rich("k", subscript("x", font=:regular), font=:italic), " (Å⁻¹)"), tellwidth=false, tellheight=false, padding=(0, 0, 0, -60))
Label(f[1:2, 1, Left()], rich(rich("k", subscript("y", font=:regular), font=:italic), " (Å⁻¹)"), tellwidth=false, tellheight=false, padding=(0, 70, 0, 0), rotation=π/2)

colgap!(f.layout, 5)
rowgap!(f.layout, -200)

save("ehh-momentum-conservation-factor-compact-3.png", f)
f