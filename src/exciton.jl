...
p = let σ = 20fs, # Note that here σ tells us the width of the pulse; it should be *large* to produce δ-function like ARPES spectrum
    ...
    ϵ = 6.4, 
    E_B = -0.1,
    w = 0.0, # The value used in the literature: 0.7,
    β = 100,
    ...
    plot!(p, k_points_near_valley, ϵ_v.(k_points_near_valley) .+ E_SQ(0),
        label = raw"$\mathbf{Q} = 0$, varying $\mathbf{k}$",
        linestyle = :dot,
...
# Plot the signature of a single exciton 
p = let σ = 20fs, # Note that here σ tells us the width of the pulse; it should be *large* to produce δ-function like ARPES spectrum
    m_v = 0.8184,
    m_c = 0.4794,
    E_g = 0.7830,
    ...
    A_kω_Q = single_exciton_arpes_signature(ϵ_v, Q, A_SQ, E_SQ, 
    broadening, k_points, ω_points)
    A_kω = A_kω_Q
